...
Because existing works mainly focus on optimizing CPU and memory utilization, network bandwidth is usually largely under-utilized.
...
In this paper, we first present BAShuffler, a bandwidth-aware shuffle scheduler that can maximize the overall network bandwidth utilization 
...
and decrease shuffle completion time by up to 29\%.
% \st{, especially in heterogeneous networks.}
\end{abstract}
...
\st{In reality, however,} \red{However, in reality,} the shuffle-heavy tasks need to transfer 
...
In \st{the} distributed platforms, the shuffle tasks by default
...
\st{Obviously, however,} \red{However, obviously,} without monitoring the actual connection
allocations %status in
in the cluster, this simple random source scheduling approach cannot offer
any bandwidth guarantee. 
\st{And} \red{It} will likely lead to \red {much} worse performance
...
A heterogeneous network environment \st{can be quite} \red{is} common in practice
...
Even in enterprise data centers, heterogeneity exists during the long period of \st{devices} \red{machine} upgrading~\cite{kant2009data}.
...
the \st{well-defined} network level model cannot fully capture the the behavior of the shuffle operation, e.g., the dynamic and casual arriving time of the fetch flows
...
% \st{We conduct experiments on both a physical cluster and an EC2
...
\red{We have done some initiatory work that shows how the proper source selection policy can improve the bandwidth utilization in shuffle operations~\cite{Liang:2016:BMN}. We extend the work in this paper with the following contributions:}
\begin{itemize}
\setlength{\itemsep}{0pt}
\setlength{\parskip}{0pt}
\setlength{\parsep}{0pt}
\item \red{We discussed in detail the difficulty of managing the network bandwidth in 
current distributed resource management platforms, and explored benefits 
of improving the shuffle bandwidth utilization by scheduling the flow source nodes at the application level.}

\item \red{We proposed the greedy source selection (GSS) algorithm 
and the partially greedy source selection (PGSS) algorithm that 
maximize the network bandwidth utilization based on the precise 
max-min fair estimation on the bandwidth utilization. 
We demonstrated that these proposed algorithms are efficient in shuffle operations.} 

\item \red{We presented BAShuffler ,which is implemented seamlessly in YARN,  
to schedule the flow source nodes at the application level based on GSS or PGSS.
Existing MapReduce-like applications can run on BAShuffler in full bandwidth utilization without any modification.}

\item \red{We conducted experiments on both a physical cluster and an EC2
virtual cluster to evaluate the real performance of BAShuffler. 
Experiment results show that BAShuffler significantly increases the
shuffle throughput
and reduce the total job completion time, by up to 29\% for
shuffle-heavy jobs in both clusters
as compared to the original YARN. 
We also used simulations to mimic the clusters of different scales. 
The simulation results  
PGSS can greatly improve the cluster network bandwidth
utilization especially in a heterogeneous network environment, with low scheduling overheads.
}
\end{itemize}
