...
\begin{abstract}
Shuffle operations are pervasive in distributed applications and consist of many network flows across computer nodes.
\st{The problem of underutilization of the network bandwidth can
...
\red{\st{
The random source selection policy in shuffle operations 
largely under-utilizes the network bandwidth and greatly affect the completion time
of shuffle tasks having large volumes of data to transfer. 
}
}
\red{The network bandwidth is usually largely under-utilized because 
the existing flow source selection policy does not consider the 
bandwidth utilization.
It greatly affects the shuffle completion time
when transferring large volumes of data}
Unfortunately, the impact of bandwidth underutilization is more pronounced
if the network bandwidth capacities of the nodes are
varied. 
\red{
This heterogeneous network setting widely exists in practice.
}


\st{In this paper, we demonstrate how shuffle flows can significantly under-utilize the network bandwidth capacity in a max-min fair network.
...
\red{
In this paper, we first present BAShuffler, a bandwidth-aware shuffle scheduler, that maximizes the overall network bandwidth utilization 
in shuffle operations.
...
that has the full picture of shuffle operations and schedules flow source nodes based on the precise bandwidth allocation estimation.
}
...
the original Hadoop.
\st{, especially in heterogeneous
networks.}
\end{abstract}
...
And it will likely lead to worse performance
