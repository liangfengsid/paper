...
\begin{abstract}
The shuffle operation, which relies on the network bandwidth to transfer data across computer nodes in the cluster, is pervasive in the distributed applications.
However, unlike the node-monopolistic resource such as memories and CPU cores, the existing distributed resource management platforms does not offer a fine-grained resource allocation mechanism for the network bandwidth.
The problem of underutilization of the network bandwidth can
greatly affect the completion time of those tasks
having large volumes of data to transfer within the cluster,
the shuffle-heavy MapReduce jobs being a typical one. 
%Unlike tasks that rely on the memory and the CPU,  whose execution time 
%only depends on the resource on the single node, the performance of the shuffle task is determined by 
%the available I/O bandwidth on both ends of the source and the destination nodes. 
%The influence of the bottleneck of bandwidth in one endpoint of a communication flow is more usual especially 
Unfortunately, the impact of bandwidth underutilization is more pronounced
if the network bandwidth capacities of the nodes in the cluster are
varied.


%In this paper, we demonstrate how the data transfer rates of the randomly scheduled communication flows 
%can be limited by the bottleneck network links and are thus lower than the network bandwidth capacity,
In this paper, we demonstrate how the communication flows with
randomly selected source nodes in the shuffle phase
cannot fully utilize the network bandwidth capacity in a max-min fair network,
%propose an algorithm to calculate the network throughput of the max-min fair network, 
and present BAShuffler, a bandwidth-aware shuffle scheduler, that can
maximize the overall network bandwidth utilization by using a greedy
or a partially greedy method 
when scheduling the source nodes of the fetch flows. 
We evaluate BAShuffler with a variety of realistic benchmarks in testbeds 
comprising a physical cluster and an EC2 virtual cluster respectively. 
Simulation is also used to evaluate the performance of BAShuffler in
clusters of different scales.
Experiment results show that BAShuffler can significantly improve the
cluster shuffle throughput
and reduce the execution time of shuffle tasks by up to 29\% as compared to
the original YARN, especially in heterogeneous network bandwidth
environments.
\end{abstract}
...
% Hadoop is a fault-tolerant, highly reliable and scalable distributing
% computing platform for big data analysis
% and processing. It has been widely used in both research and
% industrial communities since the launch of its first version (referred
% to as HadoopV1 in this paper).
% HadoopV1 fits computing tasks into the MapReduce paradigm and tries
% to schedule the tasks to run in parallel in a cluster and in a
% fault-tolerant fashion \cite{dean2008mapreduce}. 

% In HadoopV1, %the MapReduce computing paradigm runs
% the map and the reduce
% tasks of a MapReduce job are assigned a fixed number of working slots on
% each node, regardless of the actual computing resources that the tasks may
% need to consume at the node.
% To make resource allocation more dynamic and in tune
% with the needs of the tasks, the next generation of Hadoop features YARN
% \cite{vavilapalli2013apache}, a general resource managing
% platform for the cluster.
% By encapsulating the computing resources (memories and CPU cores) of
% the cluster via the abstraction of containers, %a collection of containers,
% YARN exercises a finer-grained control over the resources. 
% %By applying the term of
% With this improved paradigm, MapReduce jobs, as well as
% jobs of other distributed programming frameworks, like Spark
% \cite{zaharia2012resilient} and Dryad \cite{isard2007dryad},
% can be divided into a set subtasks and each subtask would run within
% its container having the desired amounts of memories and CPU cores. 
% It allows the user to have a more precise control of the resources, as
% opposed to the very coarse kind of concurrency as produced by the
% static slots in HadoopV1.
% %rather than roughly determining the concurrency of computing nodes by the static slots in HadoopV1.  
...
utilization, assuming that the users are able to properly specify %configure
...
The performance of these %network bandwidth required
communicative tasks can be largely %to a great degree is
%are very common and the data transfer time is mainly
...
%speed may of course affect the throughput
%in some extent, but it is rarely the bottleneck of performance due to its faster I/O speed.  

%reference to be added 
%For example, according to our observation, one third of the execution time is spent on the shuffle tasks 
%after the map tasks finish and before the reduces tasks start in the Terasort MapReduce job in YARN.
...
%realizes the importance of better utilization of the cluster network bandwidth, and 
would try to evenly distribute the workload of the network
by randomly selecting the source to fetch the map output data. 
%((Is this the default action of Yarn? Please indicate so if it is.))
...
%and thus roughly fully utilize the network bandwidth.
%However, this simple scheduling of network connections
%cannot optimize the network bandwidth utilization by monitoring the
%actual connection scheduling status in the cluster. 
Obviously, however, without monitoring the actual connection
allocations %status in
in the cluster, this simple random source scheduling approach cannot offer
any bandwidth guarantee. 
%((I don't
%understand the "monitoring ..." part. Do you mean instead of random
%selection, we monitor something and then select the nodes accordingly
%...?))
...
%Connections between a node with the larger network bandwidth capacity and a node with the smaller one will mostly  likely 
%be congested at the end with smaller bandwidth if the connections have the random sources or destinations. 
% The reason for the poor utilization of network bandwidth in heterogeneous
% networks will be explored in detail in Section \ref{section:analysis}.
...
different network speeds. %insurance. 
...
In this paper, 
% we examine the bandwidth underutilization of the
% randomly scheduling shuffle flows
% in both homogeneous and heterogeneous networks, 
% by analyzing the max-min fairness phenomenon in TCP communication.
%We find out that the network bandwidth is not evenly shared between TCP flows in the congested network links, 
%especially when the network bandwidth capacities of the links are different. 
%We will show how to compute the bandwidth share each TCP flow can gain provided the communication pattern of TCP 
%flows, regardless of the network topology. 
...
%Existing MapReduce programs can run with BAShuffler without modifying any code. 
...
shuffle throughput
%add the precise improvement here. 
%by \textcolor{red}{xxx}\%, 
...
% \item We analyzed the cause of the network bandwidth underutilization of the shuffle operation in the TCP network by using the max-min fairness allocation algorithm.
% We stressed the amplification of the underutilization problem in the heterogeneous network environment. 

% \item We presented the efficient algorithms named Greedy Source Selection (GSS) and Partially Greedy Source Selection (PGSS), which schedules the sources of the fetch flows 
% of the shuffle operation at the application level to achieve the the maximum 
% network bandwidth utilization.

% \item We implemented BAShuffler, the bandwidth-aware shuffle source scheduler, atop YARN. 
% BAShuffler leverages PGSS as the shuffle source scheduling algorithm. 
% We evaluated BAShuffler in both the physical cluster and the virtual cluster.
% \end{itemize}
...
% In Section \ref{section:analysis}, we analyze the bandwidth utilization status with different shuffle scenarios
% and discuss the rationality of scheduling the shuffle flows in the application level.
%why some TCP flows cannot use the idle network bandwidth
%at either the source or the
%destination endpoints of the connections,
%and thus cannot obtain more bandwidth share to fully utilize the
%capacities of the links concerned.
%In Section \ref{section:max-min}, we explore the method to obtain the actual network bandwidth
%utilization of the TCP flows in the max-min fair network. 
...
a convenient tools used by BAShuffler. 
...
containers
%in the cluster and will run in their respective containers with
having the required amounts of memories and CPU cores.
%% The computing resources capacity in a container are the maximum resource quotas that a task can use during its lifetime, 
%% and are not the actual resources that the task is using or will use during its lifetime. 
%% If the task resource usage exceeds the capacity of the container it runs in, the task will fail. 
...
%towards the approximate optimal practice. 
...
% Although YARN can hardly achieve dynamic optimal performance, 
% its ability to control the memories and CPU core serves as a better resource control method 
% than the static slot approach in HadoopV1. 
%A method to dynamically control the degree of concurrency in each node
%based on runtime information to achieve the optimal performance
%is introduced in \cite{smapreduce}. 
...
cluster.
%address the importance of controlling the network bandwidth in the
%Hadoop cluster but cannot raise an feasible solution. 
Indeed,
...
links being underutilized. %in terms of bandwidth.
%According to our observation, as long as the number of fetchers of
%each shuffle lies within a reasonable range,
%e.g., 2 to 5, the overall throughput will not deviate too much. 
...
% \begin{figure}[!t]
% \begin{algorithmic}[1]
% \Require $Sources$: the source nodes to select; 
% \\$Pattern$: the sources and destinations the allocated flows in the cluster;\\
% \Return $Selected$;

% \State $Heaviest$ $\gets$ the heaviest-loaded source nodes in $Sources$;
% \State $MaxBandwidth$ $\gets$ 0;

% \For{\textbf{each} Source $\in$ Heaviest}
%   \State $Util$ $\gets$ the MMF bandwidth utilization of the whole 
%   \State cluster after adding $Source$ to $Pattern$;
  
%   \If{Util $>$ MaxBandwidth}
%     \State $MaxBandwidth \leftarrow Util$;
%     \State $Selected \leftarrow Source$;
%   \EndIf
% \EndFor
% \State add $Selected$ to $Pattern$;
% \State update the load count of $Selected$;
% % \caption{Partially Greedy Source Selection}
% % \label{algo:PGSS}
...
%and we call this algorithm which applies GSS only to a set of
%pre-selected source nodes as PGSS. 

PGSS may not increase the overall bandwidth utilization as compared to GSS, 
%In fact, the bandwidth utilization after applying PGSS is usually
%smaller than applying GSS,
...
%Following the same settings of
%We will not go through PGSS and GSS with different flow arrival
%pattern here,
%but we compare their performance with simluation,
...
% \subfloat[Sparse Case]{
% \includegraphics[width=0.46\columnwidth, height=0.3\columnwidth]{figure3a}
% \label{fig:network1}}
% \subfloat[Intensive Case]{
% \includegraphics[width=0.46\columnwidth, height=0.3\columnwidth]{figure3b}
% \label{fig:network2}
% }
...
%\subsubsection{Sparse Flows}
%Fig.~\ref{fig:network1} shows
%a scenario where the shuffle communication flows are sparse.
%There are three nodes, A, B, and C, in the cluster and 
%each node has the equivalent uplink and downlink bandwidth capacity of,
%say, 6 Mbps.
%The solid arrows labeled a and b represent the existing fetch flows from A to
%C and from B to C, respectively.
%These two flows will share the bandwidth of the download link of Node
%C, each getting 3 Mbps, and the overall bandwidth utilization is 6 Mbps. 
%A shuffle on Node B has map output data to fetch on both Node A and Node C. 
%Now, a fetcher on Node B is available and needs to decide a source
%node to fetch the data.
%If the fetcher chooses to fetch data from Node A with the new Flow c1,
%as indicated by the dashed arrow,
%Flow c1 will only get a bandwidth share of 3 Mbps, and the cluster
%overall bandwidth utilization is 9 Mbps, out of the maximum capacity
%of 18 Mbps. Hence, 50\% of the bandwidth is left idle and wasted.  
%However, if the fetcher chooses to fetch data from Node C with the new Flow c2, 
%Flow c2 can occupy the full bandwidth of the downlink of Node B or uplink of Node C, which is 6 Mbps, 
%without affecting the bandwidth allocation of the other flows in the cluster. 
%The cluster overall bandwidth utilization increases to 12 Mbps, which
%is 1.3x of that when Node A is chosen.
...
% There are three nodes, A, B, and C, in the cluster and 
% each node has the equivalent uplink and downlink bandwidth capacities, 
% which are listed in Table \ref{table:networkSetting}.
...
% \renewcommand{\arraystretch}{1}
% % \extrarowheight as needed to properly center the text within the cells
% \caption{Capacities of Upload\&Download Links in Different Network Settings}
% \label{table:networkSetting}
% \centering
% % Some packages, such as MDW tools, offer better commands for making tables
% % than the plain LaTeX2e tabular which is used here.
% %\begin{tabular}{|c||c|c|}
% \begin{tabularx}{.24\textwidth}{c||c|c|c}
% \hline
% \textbf{Settings} & \textbf{A} & \textbf{B} & \textbf{C} \\
% \hline
%  Homogeneous&6&6&6\\
% \hline
% Heterogeneous &6&6&12\\
% \hline
% \end{tabularx}
% %\end{tabular}
% \end{table}
...
%In this section, we first compare the performance of GSS and PGSS
%against that of RSS using synthetic cluster settings and workloads. 
%Following that
%we show that BAShuffler has also remarkable improvement on the shuffle
%performance in YARN
%in both realistic physical and virtual clusters with
...
%Amazon Linux Image \cite{aws}. We refer to the c3.xlarge instance as the \emph{medium} instance 
%and the c3.2xlarge instance as the \emph{large} instance below. 
...
%and each of the remaining 18 instances, no matter medium or large,
%is set up as both a data node of HDFS and a node manager of YARN. 

% \begin{figure}[!t]
% \centering
% %\subfloat[Physical Terasort]{
% %\includegraphics[width=0.46\columnwidth,height=0.25\columnwidth]{r_pterasort}
% %\label{fig:r_pterasort}
% %}
% \subfloat[Terasort]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_terasort_cdf}
% \label{fig:new_terasort_cdf}
% }
% \subfloat[InvertedIndex]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_inverted_cdf}
% \label{fig:new_inverted_cdf}
% }

% \subfloat[SequenceCount]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_sequence_cdf}
% \label{fig:new_sequence_cdf}
% }
% \subfloat[RankedInvertedIndex]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_ranked_cdf}
% \label{fig:new_ranked_cdf}
% }
...
% %}
% \subfloat[Terasort]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_terasort_reduce}
% \label{fig:new_terasort_reduce}
% }
% \subfloat[InvertedIndex]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_inverted_reduce}
% \label{fig:new_inverted_reduce}
% }

% \subfloat[SequenceCount]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_sequence_reduce}
% \label{fig:new_sequence_reduce}
% }
% \subfloat[RankedInvertedIndex]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_ranked_reduce}
% \label{fig:new_ranked_reduce}
% }
...
% %}
% \subfloat[Terasort]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_terasort_speedup}
% \label{fig:new_terasort_speedup}
% }
% \subfloat[InvertedIndex]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_inverted_speedup}
% \label{fig:new_inverted_speedup}
% }

% \subfloat[SequenceCount]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_sequence_speedup}
% \label{fig:new_sequence_speedup}
% }
% \subfloat[RankedInvertedIndex]{
% \includegraphics[width=0.48\columnwidth,height=0.3\columnwidth]{new_ranked_speedup}
% \label{fig:new_ranked_speedup}
% }
...
% 2) The throughput improvement in the virtual cluster is even a bit higher
% than that in the physical cluster
% (Fig.~\ref{fig:new_completion_ratio}).
% It is probably because of the mismatch of CPU speed and the network speed in the physical cluster.  
% In the physical cluster,
% the map output speed of the specific running job is low most of the time 
% when the map tasks and the shuffle task are running in parallel, 
% which limits the shuffle speed increase that BAShuffler is capable of
% achieving. 
...
% The reduce completion time speedup and the job overall completion time
% speedup of BAShuffler as compared to RSS with different fetchers
% numbers in the virtual cluster are shown in
% Fig.~\ref{fig:r_vspeedup}.
%Generally, the performance benefit of BAShuffler is greater when the number of fetchers is smaller, 
%because RSS has a greater probability to underutilize the bandwidth in the sparse flow pattern. 
%With only two fetchers in each shuffle, 
...
%, and decrease the job
% overall completion time by as much as 38\% and 18\%, respectively.
% In the cases that the job overall completion speedup is even higher
% than the reduce completion time speedup,
% the map tasks also benefit from the higher bandwidth utilization of
% BAShuffler. 
% During the phase when the map tasks and shuffle tasks are executing
% concurrently, if the fetch flows finish at a faster rate,
% the cluster can leave more CPU time slides for the map tasks. 
...
% \begin{figure}[!t]
% \centering
% \includegraphics[width=0.8\columnwidth,height=0.4\columnwidth]{r_vspeedup}
% \caption{Reduce Completion Time Speedup (Suffixed with \_R) and Job Overall Completion time Speedup (Suffixed with \_O) of GSS and PGSS as Compared to RSS in the SequenceCount Benchmark with Different Fetcher Numbers
% in the Virtual Cluster}
% \label{fig:r_vspeedup}
% %\vspace{-0.22in}
% \end{figure}
...
% When the data size is relatively small, i.e., 100 GB, 
% the data to transfer in each shuffle are small in size and the benefit of BAShuffler is not that obvious 
% because the pattern of the fetch flows changes more frequently, which
% may amplify the scheduling overhead.
% When the shuffle size becomes large, the system overhead can be ignored, 
% and BAShuffler can achieve a higher shuffle throughput. 
% When the size of the input data is 200 GB, GSS and PGSS can decrease
% the reduce completion time by about 15\% and 13\%
% in the physical testbed, and about 25\% and 8\% in the virtual testbed. 
...
%and continuous workloads in the long term. 
%GSS and PGSS can make use the most of the bandwidth capacity.

% \begin{figure}[!t]
% \centering

% \subfloat[Homogeneous]{
% \includegraphics[width=0.8\columnwidth,height=0.45\columnwidth]{s_nodes_homo}
% \label{fig:s_nodes_homo}
% }

% \subfloat[Heterogeneous]{
% \includegraphics[width=0.8\columnwidth,height=0.45\columnwidth]{s_nodes}
% \label{fig:s_nodes_hete}
% }

% \caption{Bandwidth Utilization with Different Amount of Nodes in the Homogeneous Settings and the Heterogeneous Settings}
% \label{fig:s_nodes}
% %\vspace{-0.2in}
% \end{figure}
...
%\subsubsection{Different Numbers of Fetchers in Each Shuffle}
%The number of fetch flows somehow decides the level of workloads in the cluster.
%We change the number of fetchers in each shuffle to simulate various workloads. 
%The total node number is 20 in the heterogeneous mode. The result is shown
%in Fig.~\ref{fig:s_fetchers}.
%When the number of fetchers of each shuffle is small, say, 1 or 2, RSS
%uses only about 58\% to 74\% of the capacity,
%while the bandwidth utilizations of GSS and PGSS are about 81\% to
%96\% of the capacity.
%When the number of fetchers increases, the bandwidth utilization of
%RSS gradually increases,
%but it is still much smaller than those of GSS and PGSS, which are very
%close to the total bandwidth capacity.


%\subsubsection{Continuous Workloads}
%From the results above, we see that GSS generally performs slightly better than
%PGSS with short-term workloads.
%To verify the benefit of PGSS in the long run, we run continuous workloads on the cluster with 20 nodes 
%in the heterogeneous mode. In this workload, there is one shuffle task with 5 fetchers in each node. 
%During the workload, at every time interval, 
%the shuffle on a node will request to schedule the source nodes for
%its free fetchers
%before it is the turn for the shuffle on the next node in sequence.
%When there is no free fetcher in a shuffle, the shuffle will assume
%that a random working fetcher has finished the
%fetch, release the corresponding fetch flow, and schedule a new pending
%source node for the new free fetcher.
...
%nodes, a new shuffle will start on the same node to start to fetch
%data from all the other nodes again if it has free fetchers.
%Fig.~\ref{fig:s_longrun} shows the bandwidth utilization of different
%source selection algorithms for about 2000 time intervals.
%At first, the bandwidth utilization of GSS grows faster than that of PGSS. 
%However, as time goes on, PGSS maintains high bandwidth utilization stably, 
%while GSS fluctuates below PGSS sometimes, as shown in figure by the
%highlighting rectangles.
%Generally, RSS cannot keep fully utilizing the bandwidth, and its
%performance fluctuates a lot.
%
%\begin{figure}
%\centering
%
%\includegraphics[width=1\columnwidth,height=0.5\columnwidth]{s_longrun}
%
%\caption{Bandwidth Utilization with Continuous Workloads in the Long Run}
%\label{fig:s_longrun}
%%\vspace{-0.2in}
%\end{figure}
...
% ShuffleWatcher \cite{faraz2014shufflewatcher} schedules the locality of the map tasks and the reduce tasks so that the shuffle traffic across nodes and racks can be decreased in the multi-tenant cluster environment. 
% It also delays the reduce tasks when the network traffic is heavy.
% The shuffle scheduling is roughly in the granular of tasks instead of in flows, and it does not consider maximizing the bandwidth utilization based on the bandwidth allocation status as BAShuffler does. 
