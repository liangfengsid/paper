...
Unfortunately, existing distributed platforms (e.g., YARN~\cite{vavilapalli2013apache}) mainly focus on
...
% these distributed platforms can do very well in terms of resource
...
Their performance is heavy 
affected by the throughput of network flows, which is
% throughput of a communication flow is 
...
To improve the network bandwidth utilization, existing distributed platforms~\cite{zaharia2012resilient,vavilapalli2013apache} adopt
a random flow source node selection policy. 
...
% Shuffle tasks by default would 
...
However, 
% the pattern of network flows can be dynamic and complicated. 
% Obviously, 
...
% cannot offer any bandwidth guarantee. 
