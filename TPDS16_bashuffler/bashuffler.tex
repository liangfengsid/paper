...
\begin{abstract}
\red{
The shuffle operation is pervasive in distributed applications and consists of many network flows across computer nodes in datacenters. 
}
\st{The shuffle operation, which relies on the network bandwidth to transfer data across computer nodes in the cluster, is pervasive in the distributed applications.
...
greatly affect the completion time of shuffle tasks
...
if the network bandwidth capacities of the nodes \st{in the cluster} are
varied. 
\red{
\st{
This heterogeneous network bandwidth environment widely exists in commodity 
clusters and enterprise data centers during upgrading periods.
}
}
...
In this paper, we demonstrate how \st{the communication flows with
randomly selected source nodes in the shuffle phase
cannot fully utilize} \red{shuffle flows can significantly under-utilize} the network bandwidth capacity in a max-min fair network.
%propose an algorithm to calculate the network throughput of the max-min fair network, 
We present BAShuffler, a bandwidth-aware shuffle scheduler, that \st{can
maximize} maximizes the overall network bandwidth utilization \st{by using a greedy
or a partially greedy method 
to schedule the source nodes of the shuffle flows} in shuffle operations. 
\blue{
\st{Instead of working on the network level that schedules the bandwidth of flows or the routing of data packages, BAShuffler uses the application-level approach 
that schedules flow source nodes based on the precise bandwidth allocation estimation
without changing the underlying network. 
}
}
\red{Compared to the existing network-level solutions, 
BAShuffler takes the first step to adopt the application-level approach 
that schedules flow source nodes based on the bandwidth allocation estimation
without changing the underlying network. 
}
\red{Besides simulations,} we evaluate BAShuffler with a variety of realistic benchmarks in testbeds 
comprising a physical cluster and an EC2 virtual cluster respectively. 
\st{Simulation is also used to evaluate the performance of BAShuffler in
...
the original Hadoop, especially in \st{the} heterogeneous
networks. \st{bandwidth environment.}
\end{abstract}
...
\red{
Even in enterprise data centers, heterogeneity exists during the long period of devices upgrading~\cite{kant2009data}.
}
\st{In a virtual cluster, EC2 virtual machines could be running at
...
\st{
The shuffle information includes the shuffle-related information (e.g., the arriving patterns and the source/destination pairs of the flows) and the network-level information (e.g., the bandwidth capacity of the network links).
}
\blue{
\st{
A central scheduler in the distributed resource management platform, e.g., the Resource Manager of YARN, makes use of the shuffle information 
and schedules the flow source nodes for all the MapReduce-like programs running on this platform.
}
}
...
\red{
A central scheduler at the application level has the potential to achieve the scheduling results based on both the dynamic shuffle-oriented  information (e.g., the arriving patterns and the source/destination pairs of the flows) and the network-level information (e.g., the bandwidth capacity of the network links).
...
\st{The distributed resource management platforms like Mesos and Hadoop is also working at the application level. }
}
...
flows. 
\blue{The communication pattern mainly refers to the mapping from the the flow sources to the flow destinations.}
...
\bibitem{kavulya2010analysis}
S.~Kavulya et al., ``An analysis of traces from
  a production mapreduce cluster,'' in \emph{The 10th IEEE/ACM International
  Conference on Cluster, Cloud and Grid Computing (CCGrid)}, May 2010, pp.
  94--103.
...
F.~Ahmad et al.,
  ...
  Association, Jun. 2014, pp. 1--13. [Online]. Available:
