...
Unfortunately, managing the network bandwidth is difficult, 
and existing distributed resource management platforms, such as YARN~\cite{vavilapalli2013apache}
and Mesos~\cite{hindman2011mesos}, focus on
managing node-monopolize resources (e.g., CPU cores and memories). 
% Many popular distributed resource management platforms such as
% For example, YARN~\cite{vavilapalli2013apache} (the latest Hadoop~\cite{white2015hadoop}) and Mesos~\cite{hindman2011mesos} 
% % are developed to cope with the 
% % MapReduce-like applications with shuffle operations. 
% encapsulate the computing resources (usually memories and CPU cores) of the cluster via the abstraction of containers.
...
many compute-intensive jobs, these distributed platforms can do very well in terms of resource
utilization, assuming that the users are able to properly specify %configure
%approximately proper
the resource sizes for the tasks.
% , either by experience or by estimation. 
...
% Unfortunately, managing the network bandwidth is difficult. 
However, the network bandwidth is the shared resource of a pair of computer nodes even for one single task that transfers data between them. 
The throughput of a communication flow is mainly determined by the available network bandwidth of
...
To address this bandwidth management problem, existing distributed platforms adopts
the random flow source node selection policy. 
But the random policy is bandwidth-ignorant 
and is very likely to under-utilize the network bandwidth. 
Shuffle tasks by default
...
Worse, the bandwidth-ignorant source selection policies will cause significant bandwidth under-utilization problem
if the network is a heterogeneous one.
In heterogeneous network, the network bandwidth capacities of the links are different.
...
% \item We conducted experiments on both a physical cluster and an EC2
% virtual cluster to evaluate the real performance of BAShuffler. 
