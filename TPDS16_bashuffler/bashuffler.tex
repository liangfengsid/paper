...
to schedule the source nodes of the shuffle flows. 
\blue{
Instead of working on the network level that schedules the bandwidth of flows or the routing of data packages, BAShuffler uses the application-level approach 
...
the original YARN, especially in the heterogeneous
network bandwidth environment.
\end{abstract}
...
In a virtual cluster, EC2 virtual machines could be running at
...
\blue{
RSS is widely used in the popular distributed platforms as the only shuffle 
source selection algorithm~\cite{zaharia2012resilient}. 
}
...
The distributed resource management platforms like Mesos and Hadoop is also working at the application level. 
}
...
2) The distributed platforms can work on the ordinary operation systems and network devices. 
}
