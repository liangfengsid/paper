...
and thus applying CKP would not help much.%is trivial. 
...
in both schemes. Suppose in one of such iterations, keys of input
...
In a nutshell, if input keys are uniformly bound to mapped keys in all
iterations, the workload skewness of CKP is the same as that of RKP. 
...
that are bound to mapped key $userId$ is equal to the average number of
...
completion time of each iteration; 2) due to hashing, each executor
