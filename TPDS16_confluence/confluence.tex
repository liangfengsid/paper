...
data on many computers and generates significant network traffic.
...
% provides users a simple map-and-reduce
...
iterations reduces significantly with a predictable value. %in polynomial time,
...
different fields, 
namely MovieLensALS, GoogleAlbum, MultiAdjacentList and KMeans.
...
and the Key Dependency of Different Applications. Key Partition Binding is Applied to Keys in Dashed Rhombus Boxes
}
...
The iterative transformation of the data in algorithm MultiAdjacentList is shown in Fig.~\ref{fig:application}(c).
...
in Fig.~\ref{fig:application}(d), we have new key dependencies with 
...
Key dependency exists in each cluster iteration of the KMeans algorithm (Fig.~\ref{fig:application}(e)).
Suppose in iteration $i$, 
$pr_{ij}$ is the probability of that any point $p_i$ is grouped 
into the same cluster as it was grouped into in a previous iteration $j$ ($j < i$), where the cluster is denoted as $centroid_j(p_i)$,
...
It can be interpreted as: if a point is partitioned to a cluster 
in an iteration, 
we bind its partition to that of this cluster afterwards,
regardless of which cluster it will be actually grouped into in the following transform operations.

CKP reduces the shuffle size by a percentage of $pr_{ij}$ in each iteration after iteration $j$. 
...
The code looks like:
\begin{equation*}\label{eq:confluencePartitioner}
\begin{aligned}
data.shuffleOperation (new\ ConfluencePartitioner \\
(numPartitions, mappingFunction))
\end{aligned}
\end{equation*}
, where the ``data'' is often an RDD in Spark and the ``shuffleOperation'' is any shuffle function that repartition distributed data, such as $reduceByKey$ or $join$. 
Fig.~\ref{fig:alscode} shows the code of using ConfluencePartitioner
to apply CKP in MovieLensALS, where only the single line of code 
in bold is added to the original algorithm.
