...
\blue{\st{The application semantics of iterative distributed operations, 
...
With the knowledge of the \st{application semantics} \red{key dependency},
...
iterations based on the \st{application semantics of the keys} 
\red{key dependency relationship}.
...
\item We invented the new data structure Key Dependency Graph to depict the iterative \st{application semantics} \red{key dependency} 
...
the \st{application semantics of the keys of the} \red{key dependency of} data, which depends on the
application logic.
...
of the distributed operation. 
The programmer is supposed to be well acquainted with the distributed operation \st{semantics} \red{logic}. 
By saying that we know the \st{distributed operation semantics} \red{key dependency} for a specific  \st{application} \red{iterative distributed operation},
it means that we know what the output key-value pairs will be 
for an input key-value pair in \st{a} \red{every} computing iteration. 
We use the key dependency graph (introduced later) to portrait the \st{distributed operation semantics} \red{iterative key dependency}
so that it is comprehensible to the computer. 
Every distributed operation has its own \st{semantics} \red{key dependency}
and it relies on the programmer to understand the \st{distributed operation semantics} \red{iterative key dependency based on the application logic}
...
The key dependency is specific to the \red{code logic of the} distributed operation \st{semantics}
and \st{the distributed operation semantics} \red{it} decides which graph node should point to which others. 
The distributed operation \st{semantics} \red{code logic} should be known prior by the programmers.
...
\st{, which is constrained by the application semantics. }
...
in comparison with that $S''$ of the random key partitioning scheme (RKP), which is ignorant of the key \st{semantics} \red{dependency} and partition the data entries randomly (e.g., partition by the hashed value of keys). 
...
it can be divided into two phases based on the \st{semantics} \red{logic} of the distributed operations. 
...
it is hard for the machines to really \textbf{understand} the application \st{semantics} \red{logic} to draw the KDG. 
...
These key partitioning schemes rely not only on \st{application semantics} 
\red{the application logic} but also on the distribution of datasets, we do not compare CKP with them here. 
...
which is constructed \st{with the application semantics} 
\red{based on the iterative key dependency relationship}.
