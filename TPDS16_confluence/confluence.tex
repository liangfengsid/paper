...
data on many computers, generating possibly a significant amount
of network traffic which often dominates a job's completion time.
% and take up a substantial portion of the overall I/O workloads 
% for many MapReduce-like distributed operations. 
This traffic is particularly pronounced in iterative distributed operations
where each iteration invokes a shuffle operation. 
We observe that data of different iterations are related according to 
the transformation logic of distributed operations.
If data generated by the current 
iteration are partitioned to the computers where they will be processed
in the next iteration, unnecessary shuffle network traffic between the two iterations can be prevented.
...
We model general iterative distributed operations as the
transform-and-shuffle primitive and define a powerful notion named
Confluence key dependency to
precisely capture the data relations in the primitive.
We further find that by binding key partitions between different 
iterations based on the Confluence key dependency, 
the shuffle network traffic can always be reduced by a 
...
We implemented the Confluence system.
Confluence provides a simple interface for programmers 
to express the Confluence key dependency, 
based on which Confluence automatically generates efficient key partitioning schemes.
...
Evaluation results on diverse real-life applications show that Confluence
greatly reduces the shuffle network traffic, 
resulting in as much as 23\% job completion time reduction.
...
%to greatly reduce the shuffle network traffic.
...
Iterative map-and-reduce-like paradigms such as the in-memory %paradigm
...
key dependency) %in the rest of this paper,
...
the shuffle size can be reduced to the largest possible extent %in polynomial time 
...
\begin{algorithm}[H]
  ...
  \ForEach{key $k$}{
    ...
    $K$ = $\{ k \Rightarrow_{pr_j} f_j(k) | j = 1, 2, \ldots, \sum_j pr_j \leq 1 \}$ ; \\
...
\ForEach{iteration $i$}{
  ...
  \If{$b = True$}{
    \tcc{$K$ is a key dependency set}
    Find $\forall k, K =$ 
    $\{ k \Rightarrow_{pr_j} f_j(k) | j = 1, 2, \ldots, \sum_j pr_j \leq 1 \}$ ; \\
    Find $J$ such that $\forall j, pr_j \leq pr_{J}$; \\
...
% In unbound iterations, the key partitioning policy of CKP is the same as
% that of RKP, and the workload skewness in these iterations is the same
% in both schemes. 


% Suppose in one of such iterations, keys of input
% entries follow a distribution $Dist$.
...
% In CKP, in a binding iteration, suppose two conditions are met: 
% 1) input keys are uniformly bound to mapped keys;
% 2) the mapped keys, which are also the input keys of the next iteration,
% will be uniformly partitioned in the shuffle operation.
...
% The second condition will be eventually satisfied, as the last iteration
% always randomly partitions the keys.
% If the first condition is always true in all iterations, 
% the second condition can be proved to be always true by induction from
% the last iteration back to the previous iterations.
...
that it could be ignored.%regarding the shuffle size CKP reduces.
...
The tying can be done by hashing. %in a hash manner. 
...
completion time of each iteration; 2) \blue{with random hashing}, each executor
...
\blue{Note that the shuffle size is the compressed one after shuffle network streams serialize the raw shuffle data using \emph{Lz4}. }
...
in this benchmark. 
% Though the theoretical \emph{Lz4} compression ratio is about 2.1, 
% the compression ratio in this benchmark matches that in a Yahoo! Hadoop use report~\cite{hadoopCompression}.
...
% \bibitem{hadoopCompression}
% \BIBentryALTinterwordspacing
% ``Compression Options in Hadoop - A Tale of Tradeoffs,'' Hadoop Submit 2013  [Online]. Available:
%   \url{https://www.slideshare.net/Hadoop\\\%5FSummit/singh\%2Dkamat\%2Djune27425pmroom210c/}
% \BIBentrySTDinterwordspacing
