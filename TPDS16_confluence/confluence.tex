...
data on many computers, thus it generates significant network traffic
...
in the next iteration, unnecessary shuffle network traffic can be greatly reduced between the two iterations.
...
based on which Confluence automatically generates an efficient key partitioning scheme.
...
can greatly reduce the shuffle network traffic, 
...
In the pervasive hashed-by-key partitioning scheme, the
shuffle size of
each map-and-reduce iteration would be almost of the same size as the
map output data~\cite{dean2008mapreduce}.
...
If data generated by the current 
iteration are partitioned to the computers where they will be processed
in the next iteration, we can greatly reduce or even eliminate unnecessary network traffic of shuffle operations (we call it the
...
Compared to existing partition optimization approaches~\cite{zhou2010scope, zhang2012shuffle} 
...
To exploit the Confluence key dependency in iterative distributed operations, we present the Confluence Key Partitioning (CKP) scheme. 
Based on a set of Confluence key dependencies, CKP binds key partitions in 
different iterations and reduces the shuffle size to the maximum
extent. 
Most distributed paradigms, such as Spark~\cite{zaharia2012resilient} and Twister~\cite{ekanayake2010twister},
...
kinds of distributed applications by adding a single line of code
in the program.
...
\item \textbf{General}: We showed that CKP is easily applicable to diverse real-life distributed applications.
