...
thus it generates significant network traffic and often dominates completion time.
...
For instance, PageRank contains two types of key-value pairs: the URL key and webpage link pair, and the URL key and the webpage rank pair. The URL keys of these two pairs are relevant because if we partition them with the same URL on the same node, PageRank's \emph{join} operations in the next iteration can incur no shuffle
network traffic.
...
We present a new data structure called Key Dependency Graph (KDG) to keep track of relevant keys for general iterative distributed applications. 
