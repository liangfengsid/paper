...
import org.apache.spark.{HashPartitioner,ConfluencePartitioner}
...
object ALS extends Logging {
  ...
  def train[ID: ClassTag]( // scalastyle:ignore
    ...
    //@author liangfengsid
    val isUserIdPartition=sc.getConf.getBoolean("spark.als.userid.partitioned",false)
    val isItemIdPartition=sc.getConf.getBoolean("spark.als.itemid.partitioned",false)
    // val userPart=if(isUserIdPartition){ 
    // 	new UserIdPartitioner(numUserBlocks)
    // }else if(isItemIdPartition){
    // 	new ItemIdPartitioner(numUserBlocks)
    // }else{
    // 	new ALSPartitioner(numUserBlocks)
    // }
    	
    // val itemPart=if(isUserIdPartition){
    // 	new UserIdPartitioner(numItemBlocks)
    // }else if(isItemIdPartition){
    // 	new ItemIdPartitioner(numItemBlocks)
    // }else{
    // 	new ALSPartitioner(numItemBlocks)
    // }

    val userFunc= (a:Any) =>{
      a match{
        case userItem:(_,_) => userItem._1
        case _ => a
      }
    }
    ...
    val itemFunc= (a:Any) =>{
      a match{
        case userItem:(_,_) => userItem._2
        case _ => a
      }
    }

    val userPart=if(isUserIdPartition){ 
      new ConfluencePartitioner(numUserBlocks,userFunc)
    }else if(isItemIdPartition){
      new ConfluencePartitioner(numUserBlocks,userFunc)
    }else{
      new ALSPartitioner(numUserBlocks)
    ...
    val itemPart=if(isUserIdPartition){
      new ConfluencePartitioner(numItemBlocks,itemFunc)
    }else if(isItemIdPartition){
      new ConfluencePartitioner(numItemBlocks,itemFunc)
    }else{
      new ALSPartitioner(numItemBlocks)
...
//    }.groupByKey().mapValues { blocks =>
	  }.groupByKey(srcPart).mapValues { blocks =>
  ...
  class UserIdPartitioner(partitions: Int) extends org.apache.spark.Partitioner {
	  ...
	  override def getPartition(key: Any): Int = key match {
	    case null => 0
	    case userId:Int => {
	    	nonNegativeMod(userId.hashCode, numPartitions)
	    }
	    case userItem:(_,_)=>{
		    val index=userItem._1
	...
	class ItemIdPartitioner(partitions: Int) extends org.apache.spark.Partitioner {
	  ...
	  override def getPartition(key: Any): Int = key match {
	    case null => 0
	    case itemId:Int => {
	    	nonNegativeMod(itemId.hashCode, numPartitions)
	    }
	    case userItem:(_,_)=>{
		val index=userItem._2
		nonNegativeMod(index.hashCode, numPartitions)
	    }
	    case _ => 0
	  }
